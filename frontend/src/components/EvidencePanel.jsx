...
function getSourceName(file) {
  return sourceNames[file] || "工业知识文档";
}
...
function EvidencePanel({ result }) {
  if (!result) {
    return (
      <Card title="回答依据">
        <Empty
          description="发送问题后将在这里显示知识库检索依据"
        />
      </Card>
    );
  }
  ...
  return (
    <Card title="回答依据">
      <Space wrap size={[8, 8]}>
        <Tag color="blue">
          Tool: {result.tool_used || "none"}
        </Tag>

        <Tag color="geekblue">
          Retriever: {result.retriever_type || "none"}
        </Tag>
                ...
                items={[
                  {
                    ...
                    children: (
                      <div className="technical-details">
                        ...
                        <div>
                          Raw Score：
                          {typeof source.raw_score === "number"
                            ? source.raw_score.toFixed(4)
                            : "-"}
                        </div>

                        <div>
                          Rule Boost：
                          {typeof source.boost === "number"
                            ? source.boost.toFixed(4)
                            : "-"}
                        </div>
